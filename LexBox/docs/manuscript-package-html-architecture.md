...
3. 宿主维护 `layout.tokens.json` 作为图文全局排版 token
4. 宿主把用户工具栏调整写入 `manifest.richpostTypography`
5. 宿主维护 `masters/*.master.html` 作为 cover / body / ending 母版
6. 宿主根据当前 block、配图和 `richpostTypography` 生成新的 `richpost-page-plan.json`
7. 连续三个空行会先切出明确分页段
8. 宿主按 `tokens + master + page-plan` 合成每个 `pages/page-xxx.html`
9. 宿主生成 `layout.html` 作为多页总预览
10. 前端 iframe 直接预览真实文件
