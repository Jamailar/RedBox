...
Phase 5 为 `LexBox` 增加了受限程序化执行层，用于把机械多步读取和整理压缩成一次脚本执行。
...
- 内置 `lexbox_script_v1`：不依赖外部解释器，能直接走宿主 capability、workspace 边界和 checkpoint 体系。
...
脚本版本固定为 `lexbox_script_v1`。

```json
{
  "version": "lexbox_script_v1",
  "steps": [
    {
      "op": "tool",
      "tool": "app.query",
