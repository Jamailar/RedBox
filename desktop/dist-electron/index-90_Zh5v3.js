...
const main = require("./main-DclRpR9o.js");
require("fs/promises");
require("path");
class AdvisorChatService extends events.EventEmitter {
  constructor(config) {
    ...
    this.toolExecutor = new main.ToolExecutor(
      this.toolRegistry,
      async () => {
        const { ToolConfirmationOutcome } = await Promise.resolve().then(() => require("./main-DclRpR9o.js")).then((n) => n.toolRegistry);
...
const DIRECTOR_INTRODUCTION_PROMPT = `你是智囊团的总监，负责主持团队讨论。

## 你的角色

你是一个专业的讨论主持人，不是问题的直接回答者。你的任务是：
1. 理解用户的问题核心
2. 分析这个问题涉及的多个维度
3. 为团队成员设定讨论方向
4. 提出引导性问题激发思考
...
- 不要直接回答问题，而是引导讨论
- 根据问题类型调整讨论维度`;
const DIRECTOR_SUMMARY_PROMPT = `你是智囊团的总监，现在需要对团队讨论进行总结。
...
class DirectorAgent extends events.EventEmitter {
   ...
   * 发起讨论 - 分析用户问题并设定讨论方向
   */
  async introduceDiscussion(userMessage, advisorNames) {
    this.abortController = new AbortController();
    const signal = this.abortController.signal;
    try {
      ...
      const llm = this.createLLM();
      const messages = [
        new main.SystemMessage(DIRECTOR_INTRODUCTION_PROMPT),
...
class DiscussionFlowService extends events.EventEmitter {
   ...
   * 执行完整的讨论流程
   */
  async orchestrateDiscussion(roomId, userMessage, advisors, existingHistory = []) {
    var _a, _b;
    this.abortController = new AbortController();
    const newMessages = [];
...
${m.content}`
   ...
   * 总监开场分析
   */
  async directorIntroduction(userMessage, advisorNames) {
    ...
    return await director.introduceDiscussion(userMessage, advisorNames);
  }
  /**
   * 成员发言
   */
  async advisorSpeak(advisor, userMessage, history) {
    const advisorConfig = {
      apiKey: this.config.apiKey,
      baseURL: this.config.baseURL,
      model: this.config.model,
      ...
      advisorName: advisor.name,
      advisorAvatar: advisor.avatar,
      systemPrompt: this.enhanceSystemPrompt(advisor.systemPrompt, history),
   ...
   * 增强系统提示词，加入上下文感知
   */
  enhanceSystemPrompt(basePrompt, history) {
