...
const main = require("./main-DdLUy6zV.js");
require("fs/promises");
require("path");
class AdvisorChatService extends events.EventEmitter {
  constructor(config) {
    ...
    this.toolExecutor = new main.ToolExecutor(
      this.toolRegistry,
      async () => {
        const { ToolConfirmationOutcome } = await Promise.resolve().then(() => require("./main-DdLUy6zV.js")).then((n) => n.toolRegistry);
...
class DiscussionFlowService extends events.EventEmitter {
   ...
   */
  async orchestrateDiscussion(roomId, userMessage, advisors, existingHistory = [], isSixHatsMode = false) {
    var _a, _b, _c;
    this.abortController = new AbortController();
    const newMessages = [];
