...
async function checkNeedsAutoIndex(advisorId, advisorsDir) {
  const knowledgeDir = path__namespace.join(advisorsDir, advisorId, "knowledge");
  const indexFile = path__namespace.join(knowledgeDir, "embeddings.json");
  try {
    const files = await fs__namespace.readdir(knowledgeDir).catch(() => []);
    const textFiles = files.filter((f) => f.endsWith(".txt") || f.endsWith(".md"));
    if (textFiles.length === 0) {
      return { needsIndex: false, reason: "无知识库文件", newFiles: [], modifiedFiles: [] };
    }
    let index = null;
    try {
      const content = await fs__namespace.readFile(indexFile, "utf-8");
    ...
    const indexedSources = new Set(index.chunks.map((c) => c.source));
    const newFiles = [];
    const modifiedFiles = [];
    for (const file of textFiles) {
      if (!indexedSources.has(file)) {
        ...
        const filePath = path__namespace.join(knowledgeDir, file);
        const stats = await fs__namespace.stat(filePath);
        const fileLastModified = stats.mtimeMs;
        const fileChunks = index.chunks.filter((c) => c.source === file);
        const indexLastModified = Math.max(...fileChunks.map((c) => c.lastModified || 0));
        if (fileLastModified > indexLastModified) {
    ...
    if (newFiles.length > 0 || modifiedFiles.length > 0) {
      return {
        needsIndex: true,
        reason: newFiles.length > 0 ? `${newFiles.length} 个新文件` : `${modifiedFiles.length} 个文件已修改`,
        newFiles,
        modifiedFiles
      };
    }
    return { needsIndex: false, reason: "索引已是最新", newFiles: [], modifiedFiles: [] };
  } catch (error) {
    console.error("[checkNeedsAutoIndex] Error:", error);
    return { needsIndex: false, reason: "检查失败", newFiles: [], modifiedFiles: [] };
  }
}
exports.buildVectorIndex = buildVectorIndex;
exports.checkNeedsAutoIndex = checkNeedsAutoIndex;
