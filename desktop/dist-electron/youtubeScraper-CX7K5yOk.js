...
async function installYtdlp(onProgress) {
  return new Promise((resolve, reject) => {
    ...
    const file = fs.createWriteStream(LOCAL_YTDLP_PATH);
    https.get(YTDLP_URL, (response) => {
      if (response.statusCode === 302 || response.statusCode === 301) {
        https.get(response.headers.location, (redirectResponse) => {
          handleDownload(redirectResponse, file, resolve, reject, onProgress);
        });
      } else {
        handleDownload(response, file, resolve, reject, onProgress);
      }
    }).on("error", (err) => {
      fs.unlink(LOCAL_YTDLP_PATH, () => {
      });
      reject(err);
    });
  });
}
function handleDownload(response, file, resolve, reject, onProgress) {
