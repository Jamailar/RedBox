...
export async function getRedClawProject(projectId: string): Promise<{ project: RedClawProject; projectDir: string }> {
  const projectPath = resolveProjectJsonPath(projectId);
...
export async function saveRedClawCopyPack(input: {
  ...
  const { projectDir, project } = await getRedClawProject(input.projectId);
  ...
  const nextProject = await updateProjectStatus(input.projectId, 'drafted');
...
export async function saveRedClawImagePack(input: {
  ...
  const { projectDir } = await getRedClawProject(input.projectId);
...
export async function saveRedClawRetrospective(input: {
  ...
  const nextProject = await updateProjectStatus(input.projectId, 'reviewed');
