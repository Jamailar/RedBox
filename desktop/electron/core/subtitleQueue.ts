...
class SubtitleDownloadQueue {
    ...
    private readonly MIN_INTERVAL_MS = 3000; // 最小间隔 3 秒

    /**
     * 添加字幕下载任务到队列
     */
    addTask(videoId: string, outputDir: string): Promise<{ success: boolean; subtitleFile?: string; error?: string }> {
    ...
    private async processQueue() {
        ...
        while (this.queue.length > 0) {
            ...
            const waitTime = Math.max(0, this.MIN_INTERVAL_MS - timeSinceLastDownload);
...
export async function queueSubtitleDownload(
    ...
    return subtitleQueue.addTask(videoId, outputDir);
}
