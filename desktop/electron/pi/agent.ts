...
class AgentManager {
  ...
  private createAgent(config: PiAgentConfig): Agent {
    ...
    const baseURL = config.baseURL || (settings.api_endpoint as string) || (settings.openaiApiBase as string) || 'https://api.openai.com/v1';
...
请始终用中文回复，除非用户要求用其他语言。`;
    ...
    const agent = new Agent({
      initialState: {
        model: this.createModelWithBaseUrl(modelName, baseURL),
        thinkingLevel: 'off',
      },
